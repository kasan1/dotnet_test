...
namespace Agro.Integration.Logic.OutService.GKB
{
    public class GKBLogic : IGKBLogic
    {
        ...
        public async Task<byte[]> GetGKBFile(string iin, string reportName)
        {
            var doc = new XmlDocument();
            doc.LoadXml($"<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:rep=\"http://report.chdb.scb.kz\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">" +
                $"<soapenv:Header><userId>{_conf.Value.Integrations.GKB.UserId}</userId></soapenv:Header>" +
                $"<soapenv:Body><rep:requestCreditReport><reportRequest><generationParams><language>ru</language>" +
                $"<outputFormats>PDF</outputFormats><reportType>{reportName}</reportType>" +
                $"</generationParams><reportParams xsi:type=\"rep:personCreditReportParams\">" +
                $"<new>true</new><iin>{iin}</iin><consentConfirmed>true</consentConfirmed></reportParams>" +
                $"</reportRequest></rep:requestCreditReport></soapenv:Body></soapenv:Envelope>");
            var httpContent = new StringContent(doc.InnerXml.ToString(), Encoding.UTF8);
            var _httpClient = _httpClientFactory.CreateClient(Enum.GetName(typeof(IntegrationType), IntegrationType.GKB));
            var _ = await _httpClient.PostAsync(_conf.Value.Integrations.GKB.Url, httpContent);
            ...
            try
            {
                ...
                if (_.StatusCode == HttpStatusCode.OK)
                {
                    return await _repo.Add(new Shared.Data.Context.OutService
                    {
                        IIN = iin,
                        ResponseContent = xml,
                        Type = Shared.Data.Primitives.OutServiceEnum.GKBNew
                    });
                }
                else
                {
    ...
    //    public T ConvertNode<T>(XmlNode node) where T : class
    //    {
